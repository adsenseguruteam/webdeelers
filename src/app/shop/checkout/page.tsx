...
export default async function CheckoutPage() {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        const convertedPrice = convertPrice(
          ...
          response.data.product.currency, 
          userCurrency
        );
  ...
  const handleBuy = async () => {
    ...
    if (selectedPaymentMethod === "upi" || selectedPaymentMethod === "binance") {
  ...
  const submitTransactionId = async () => {
    ...
    try {
      ...
      const orderAmount = couponData ? couponData.finalAmount : displayPrice;
      ...
      if (response.data.success) {
        toast.success("Payment submitted successfully! Redirecting to your orders...");
  ...
  const finalAmount = couponData ? couponData.finalAmount : displayPrice;
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-100 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-slate-500 mb-6">The product you're trying to purchase doesn't exist.</p>
          <Link href="/shop">
              ...
              <CardContent className="p-6">
                <div className="flex gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-slate-600 text-sm capitalize mt-1">{product.category}</p>
                    <div className="mt-3">
                      ...
                      {product.comparePrice && product.comparePrice > product.price && (
                        <span className="text-lg text-slate-400 line-through ml-2">
                          {userCurrency} {convertPrice(product.comparePrice, product.currency, userCurrency).toFixed(2)}
                        </span>
                      )}
                    </div>
                  </div>
                    ...
                    <div className="mt-3 text-sm text-emerald-700">
                      <p>
                        {couponData.discountType === 'percentage' 
                          ...
                          : `${userCurrency} ${couponData.discountValue} discount`}
                      </p>
                      <p className="font-medium">You save: {userCurrency} {couponData.discountAmount.toFixed(2)}</p>
                    </div>
                  </div>
                ) : (
          ...
          <div className="space-y-6">
            ...
            <Card className="border-slate-200 shadow-sm sticky top-8">
              <CardHeader className="bg-gradient-to-r from-slate-50 to-slate-100 border-b border-slate-200">
                <CardTitle className="text-slate-900">Order Total</CardTitle>
              </CardHeader>
              <CardContent className="p-6 space-y-6">
                ...
                <div>
                  ...
                  <div className="space-y-3">
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${
                          selectedPaymentMethod === "upi" ? "border-blue-500 bg-blue-500" : "border-slate-300"
                        }`}>
                          {selectedPaymentMethod === "upi" && (
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-slate-600">Instant payment via UPI apps</p>
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${
                          selectedPaymentMethod === "binance" ? "border-yellow-500 bg-yellow-500" : "border-slate-300"
                        }`}>
                          {selectedPaymentMethod === "binance" && (
                            <div className="w-2 h-2 rounded-full bg-white"></div>
                          )}
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-slate-600">Credit/Debit cards (Coming soon)</p>
                          </div>
                        </div>
                      </div>
                    </div>
                ...
                <div className="space-y-3">
                  <div className="flex justify-between text-slate-600">
                    <span>Subtotal</span>
                    <span>{userCurrency} {displayPrice.toFixed(2)}</span>
                  </div>
                  
                  {discountAmount > 0 && (
                    <div className="flex justify-between text-emerald-600">
                      <span>Discount</span>
                      <span>- {userCurrency} {discountAmount.toFixed(2)}</span>
                    </div>
                  )}
                  ...
                  <div className="border-t border-slate-200 pt-3">
                    <div className="flex justify-between text-lg font-bold text-slate-900">
                      <span>Total</span>
                      <span>{userCurrency} {finalAmount.toFixed(2)}</span>
                    </div>
                  </div>
                </div>
                ...
                >
                  ...
                  {selectedPaymentMethod ? `Proceed with ${selectedPaymentMethod === "upi" ? "UPI" : "Binance"}` : "Select Payment Method"}
                </Button>
        ...
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <p className="text-sm text-slate-600">
              Pay {userCurrency} {finalAmount.toFixed(2)} using {selectedPaymentMethod === "upi" ? "UPI" : "Binance"}
            </p>
          </DialogHeader>
          <div className="space-y-6 py-4">
            ...
            <div className="bg-slate-50 rounded-xl p-4">
              ...
              {selectedPaymentMethod === "upi" && (
                <div className="space-y-4">
                  ...
                  <div className="text-center">
                    <p className="text-sm text-slate-600 mb-3">Scan QR Code to Pay</p>
                    ...
                    <p className="text-xs text-slate-500 mt-2">Amount: {userCurrency} {finalAmount.toFixed(2)}</p>
                  </div>
                </div>
              )}
              ...
              {selectedPaymentMethod === "binance" && (
                <div className="space-y-4">
                  ...
                  <div className="text-center">
                    ...
                    <p className="text-xs text-slate-500 mt-2">Amount: {userCurrency} {finalAmount.toFixed(2)} USDT</p>
                  </div>
                </div>
              )}
            </div>
            ...
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-2 required">
                Transaction ID/Reference <span className="text-rose-500">*</span>
              </label>
              <Input
                value={transactionId}
                ...
                required
              />
              <p className="text-xs text-slate-500 mt-1">Enter the transaction ID from your payment app (required)</p>
            </div>
