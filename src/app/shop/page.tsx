				...
				<div className="max-w-7xl mx-auto px-4 md:px-6 lg:px-8">
					<div className="text-center max-w-2xl mx-auto">
						...
						<p className="text-slate-300 mb-8">
							We offer custom development services tailored to your specific needs. Let's build something amazing together.
						</p>
			...
			<Card className={`group bg-white border-slate-200 overflow-hidden transition-all duration-300 hover:shadow-2xl hover:-translate-y-2 cursor-pointer ${product.isFeatured ? 'ring-2 ring-emerald-500/20' : ''}`}>
				...
				<div className="relative h-48 overflow-hidden bg-gradient-to-br from-slate-100 to-slate-200">
					...
					<div className="absolute top-3 right-3">
						<div className={`px-3 py-1 rounded-full bg-gradient-to-r ${getCategoryColor(product.category)} text-white text-xs font-medium shadow-lg`}>
							{product.category}
						</div>
					</div>
				...
				<CardContent className="p-5">
					{/* Category */}
					<p className="text-xs text-slate-500 uppercase tracking-wide mb-2">{product.category}</p>
					...
					<div className="flex items-center gap-4 mb-4">
						<div className="flex items-center gap-1">
							...
							<span className="text-sm font-medium text-slate-700">{product.rating?.average?.toFixed(1) || "0.0"}</span>
							<span className="text-xs text-slate-400">({product.rating?.count || 0})</span>
						</div>
						<span className="text-slate-300">|</span>
						<span className="text-sm text-slate-500">{product.salesCount || 0} sold</span>
					</div>
