...
export async function POST(request) {
	try {
		...
		const plan = await Plan.findOne({ name: { $regex: new RegExp(`^${user.currentPlan}$`, "i") } });
		...
		if (!plan) {
			...
			// Default to Free: 1 per week
             ...
             return NextResponse.json({ success: false, message: "Plan configuration not found." }, { status: 400 });
		}
		...
		if (plan.frequency === 'daily') {
			if (!user.lastPostDate || new Date(user.lastPostDate).toDateString() !== now.toDateString()) {
				canPost = true;
			}
		} else {
			// Weekly
			const oneWeek = 7 * 24 * 60 * 60 * 1000;
			if (!user.periodStartDate || (now - new Date(user.periodStartDate) > oneWeek)) {
				// Reset period
				user.postCount = 0;
