								...
								<table className="w-full">
									<thead className="bg-slate-50 border-b border-slate-200">
										<tr>
											...
											<th className="text-left px-6 py-4 text-sm font-medium text-slate-600">Payment</th>
										...
										{orders.map((order) => (
											<tr key={order._id} className="hover:bg-slate-50">
												<td className="px-6 py-4">
													<span className="font-mono text-sm text-slate-900">{order.orderId}</span>
												</td>
												<td className="px-6 py-4">
													<div>
												...
												<td className="px-6 py-4">
													<div className="flex items-center gap-3">
														...
														<span className="text-sm text-slate-900 line-clamp-1 max-w-[150px]">
															{order.productSnapshot?.title || "N/A"}
														</span>
													</div>
												</td>
												<td className="px-6 py-4">
							...
							<div className="space-y-6">
								...
								<div className="flex items-center justify-between p-4 bg-slate-50 rounded-xl">
									<div>
										...
										<p className="font-mono font-medium text-slate-900">{viewingOrder.orderId}</p>
									</div>
								...
								<div className="flex gap-4">
									{viewingOrder.productSnapshot?.thumbnail ? (
										<img
											src={viewingOrder.productSnapshot.thumbnail}
											alt=""
									...
									<div className="flex-1">
										<h3 className="font-semibold text-slate-900 mb-1">
											{viewingOrder.productSnapshot?.title || "N/A"}
										</h3>
										{(viewingOrder.productSnapshot as any)?.category && (
											<p className="text-sm text-slate-500 mb-2">
												Category: {(viewingOrder.productSnapshot as any).category}
											</p>
										)}
										<div className="flex items-center gap-4">
											<span className="text-lg font-bold text-slate-900">
												{formatCurrency(viewingOrder.finalAmount, viewingOrder.currency)}
											</span>
											{(viewingOrder.productSnapshot as any)?.comparePrice && (viewingOrder.productSnapshot as any).comparePrice > ((viewingOrder.productSnapshot as any)?.price || 0) && (
												<span className="text-sm text-slate-400 line-through">
													{formatCurrency((viewingOrder.productSnapshot as any).comparePrice, viewingOrder.currency)}
												</span>
											)}
										</div>
									</div>
								...
								<div className="border-t border-slate-200 pt-4">
									...
									<div className="grid grid-cols-2 gap-4 text-sm">
										<div>
											...
											<p className="font-medium text-slate-900">{viewingOrder.user?.name || "N/A"}</p>
										</div>
										<div>
											<p className="text-slate-500">Email</p>
											<p className="font-medium text-slate-900">{viewingOrder.user?.email || "N/A"}</p>
										</div>
									</div>
								</div>
									...
									<div className="space-y-2 text-sm">
										...
										{viewingOrder.paidAt && (
											<div className="flex justify-between">
												...
												<span className="text-slate-900">{formatDate(viewingOrder.paidAt)}</span>
											</div>
										)}
									</div>
								</div>
								...
								{/* Order Timeline */}
								<div className="border-t border-slate-200 pt-4">
									<h4 className="font-medium text-slate-900 mb-3">Order Timeline</h4>
									<div className="space-y-2 text-sm">
										<div className="flex justify-between">
											<span className="text-slate-500">Order Placed</span>
											<span className="text-slate-900">{formatDate(viewingOrder.createdAt)}</span>
										</div>
										{viewingOrder.deliveredAt && (
											<div className="flex justify-between">
												<span className="text-slate-500">Delivered</span>
												<span className="text-slate-900">{formatDate(viewingOrder.deliveredAt)}</span>
											</div>
										)}
									</div>
								</div>
